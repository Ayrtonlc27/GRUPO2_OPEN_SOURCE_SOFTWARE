...
@Component({
  ...
  standalone: true,
  imports: [MatIcon,MatProgressSpinner, NgIf,CommonModule,MatTableModule,MatMenu, MatButtonModule,MatChipsModule,MatMenuModule],
...
export class AlertasComponent  implements OnInit{
  ...
  editarAlerta(alerta: Alerta): void {  
    ...
    dialogRef.afterClosed().subscribe(resultado => {  
      if (resultado) {  
        this.alertasService.actualizarAlerta(resultado.id, resultado).subscribe({  
          next: () => {  
            this.cargarAlertas();  
          },  
          error: (err) => {  
